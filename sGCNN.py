...
class Graph_ConvNet_LeNet5(nn.Module):
    ...
    def graph_conv_cheby(self, x, cl, L, Fout, K):
        ...
        BxV, Fin = x.size()        
        BxV, Fin = int(BxV), int(Fin) 

        # rescale Laplacian already done 
        # lmax = lmax_L(L)
        # L = rescale_L(L, lmax) 
    ...
    def forward(self, x, d, L0,L2):
        
        # graph CL1
        #x = x.unsqueeze(2) # B x V x Fin=1  

        x = self.graph_conv_cheby(x, self.cl1, L0, self.CL1_F, self.CL1_K)
        x = F.relu(x)
        x = self.graph_max_pool(x, 4)
        
        # graph CL2
        x = self.graph_conv_cheby(x, self.cl2, L2, self.CL2_F, self.CL2_K)
        x = F.relu(x)
        x = self.graph_max_pool(x, 4)
