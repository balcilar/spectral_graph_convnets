...
class cgcnn(object):
    ...
    def __init__(self, L, F, K, p, M, filter='chebyshev5', brelu='b1relu', pool='mpool1',
                ...
                dir_name='',feature_size=1,max_node=100):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('inputs'):
                ...
                self.ph_data = tf.placeholder(tf.float32, (self.batch_size, M_0, self.feature_size), 'data')
                self.ph_laplac = tf.placeholder(tf.float32, (self.batch_size,M_0 , M_0), 'laplac')
                self.ph_labels = tf.placeholder(tf.int32, (self.batch_size), 'labels')
                self.ph_nnodes = tf.placeholder(tf.int32, (self.batch_size), 'nnodes')
                self.ph_dropout = tf.placeholder(tf.float32, (), 'dropout')

            # Model.
            op_logits = self.inference(self.ph_data, self.ph_laplac,self.ph_nnodes, self.ph_dropout)
    ...
    def inference(self, data, laplac,nnodes, dropout):
        
        # Graph convolutional layers.
        #x = tf.expand_dims(data, 2)  # N x M x F=1
        x=data
        for i in range(len(self.p)):
            with tf.variable_scope('conv{}'.format(i+1)):
                with tf.name_scope('filter'):
                    #x = self.filter(x, self.L[i], self.F[i], self.K[i])
                    x = self.filter(x, laplac, nnodes, self.F[i], self.K[i])
    ...
    def chebyshev5(self, x, L, nnode,Fout, K):
